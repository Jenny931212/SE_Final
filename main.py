...
async def project_create(request: Request, title: str = Form(...), description: str = Form(...), conn = Depends(getDB)):
    user = await current_user(request, conn)    #從 session 讀取目前登入者的資訊
    ...
    async with conn.cursor(row_factory=dict_row) as cur:
        ...
        await cur.execute("""
            ...
            VALUES (%s,%s,'open',%s) RETURNING id;
...
async def project_edit(request: Request, project_id: int,
    ...
    async with conn.cursor() as cur:
        await cur.execute("UPDATE projects SET title=%s, description=%s, updated_at=NOW() WHERE id=%s AND client_id=%s;",
...
async def submit_proposal_page(request: Request, project_id: int, conn = Depends(getDB)):
    ...
    if not user or user["role"] != "contractor":
        return RedirectResponse("/login")
    #從DB抓出這個案件的資料(id、標題和狀態)
    async with conn.cursor(row_factory=dict_row) as cur:
        await cur.execute("SELECT id, title, status FROM projects WHERE id=%s;", (project_id,))
        project = await cur.fetchone()
...
async def submit_proposal(request: Request, project_id: int,
                          message: str = Form(...), price: float = Form(...),
                          conn = Depends(getDB)):
    user = await current_user(request, conn)
    if not user or user["role"] != "contractor":
        return RedirectResponse("/login")
    async with conn.cursor(row_factory=dict_row) as cur:
        ...
        #把接案人提出意願的相關資料寫進DB
        await cur.execute("""
            INSERT INTO proposals (project_id, contractor_id, message, price)
            VALUES (%s,%s,%s,%s);
...
async def upload_closure(request: Request, project_id: int, file: UploadFile = File(...), conn = Depends(getDB)):
    user = await current_user(request, conn)
    if not user or user["role"] != "contractor":
        return RedirectResponse("/login")
    async with conn.cursor(row_factory=dict_row) as cur:
        await cur.execute("SELECT contractor_id FROM projects WHERE id=%s;", (project_id,))
        row = await cur.fetchone()
        if not row:
            raise HTTPException(404, "找不到案件")
        ...
        if contractor_id != user["id"]:
            raise HTTPException(403, "無權限")
        content = await file.read()     #讀入整個檔案內容
        dest = UPLOAD_DIR / f"project{project_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}_{file.filename}"    #儲存檔案的路徑
        with dest.open("wb") as f:  #以二進位寫入模式開啟檔案，寫完後自動關閉
            f.write(content)        #把剛剛讀進來的 bytes 資料存成實體檔案
        #把檔案的檔名、路徑等資料寫入DB，並更新案件狀態
        await cur.execute("""
            INSERT INTO closure_files (project_id, contractor_id, filename, filepath)
            VALUES (%s,%s,%s,%s);
        """, (project_id, user["id"], file.filename, str(dest)))
        await cur.execute("UPDATE projects SET status='submitted', updated_at=NOW() WHERE id=%s;", (project_id,))
        await conn.commit()
    return RedirectResponse(f"/projects/{project_id}", status_code=status.HTTP_302_FOUND)   #回到案件詳情畫面
#刪除上傳的檔案
@app.post("/files/{closure_id}/delete")
async def delete_closure_file(closure_id: int, request: Request, conn = Depends(getDB)):  
    user = await current_user(request, conn)
    if not user:
        return RedirectResponse("/login")
    async with conn.cursor(row_factory=dict_row) as cur:
        #從DB抓出這個檔案以及該案件的相關資料(委託人、接案人、案件狀態)
        await cur.execute(
            """
            SELECT cf.id, cf.filepath, cf.project_id, cf.contractor_id,
                   p.client_id, p.contractor_id AS project_contractor_id, p.status
            FROM closure_files cf
            JOIN projects p ON p.id = cf.project_id
            WHERE cf.id = %s;
            """,
        ...
        if not row:
            raise HTTPException(404, "檔案不存在")
        #只允許「該檔案的上傳者（接案人）」刪除
        if user["id"] != row["contractor_id"]:
            raise HTTPException(403, "無權限刪除此檔案")
        #如果專案已結案就不能刪除
        if row["status"] == "closed":
            raise HTTPException(400, "專案已結案，無法刪除檔案")
        #先刪實體檔
        path = row["filepath"]
        if os.path.exists(path):
            try:
                ...
                pass
        #刪DB記錄
        await cur.execute("DELETE FROM closure_files WHERE id=%s;", (closure_id,))
        #若原狀態是submitted，檢查是否還有其他結案檔，如果沒有就把狀態改回 in_progress
        if row["status"] == "submitted":
            await cur.execute("SELECT 1 FROM closure_files WHERE project_id=%s LIMIT 1;", (row["project_id"],))
            remain = await cur.fetchone()
            if not remain:
                await cur.execute(
                    "UPDATE projects SET status='in_progress', updated_at=NOW() WHERE id=%s;",
                    (row["project_id"],)
                )
        await conn.commit()
    return RedirectResponse(f"/projects/{row['project_id']}", status_code=status.HTTP_302_FOUND)
